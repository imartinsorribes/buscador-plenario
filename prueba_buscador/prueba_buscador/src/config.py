...
def load_config(path=None) -> SimpleNamespace:
    ...
    cfg = _to_ns(data)
    cfg.root = ROOT
    # PLENO_LLM_MODEL: cambia el modelo LLM sin tocar config.yaml (para el servidor de la UV
    # vía túnel, junto con PLENO_OLLAMA_HOST). Sin la variable, se usa el del yaml (local).
    import os
