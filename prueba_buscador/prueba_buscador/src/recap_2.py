...
_LLM_HOST = "http://localhost:11434"
...
def _llm(prompt: str, cfg, num_predict: int = 400, timeout: int = 600) -> str:
    ...
    payload = {
        "model": cfg.llm.model, "prompt": prompt, "stream": False, "think": False,
        "keep_alive": getattr(cfg.llm, "keep_alive", "30m"),
        "options": {"temperature": 0.2, "num_ctx": 4096, "num_predict": num_predict},
    }
    req = urllib.request.Request(
        _LLM_HOST + "/api/generate", data=json.dumps(payload).encode("utf-8"),
...
def retrieve(question: str, cfg, where, final_k: int, pool: int | None = None) -> list[dict]:
...
def build_recap(kind: str, value: str, cfg=None, con=None,
                question: str | None = None, final_k: int | None = None) -> Recap:
    ...
    if kind == "candidato":
        ...
        frags = retrieve(question, cfg, _where(speaker=speaker), final_k) if question else []
        return Recap(kind, speaker, ficha, frags, question, note)
    ...
    if kind == "partido":
        ...
        frags = retrieve(question, cfg, _where(party=party), final_k) if question else []
        return Recap(kind, party, ficha, frags, question, note)

    if kind == "pleno":
        ficha = diarios.pleno_ficha(con, value)
        if ficha is None:
            return Recap(kind, value, {}, note=f"No tengo indexado el pleno «{value}».")
        frags = retrieve(question, cfg, _where(pleno_id=value), final_k) if question else []
        return Recap(kind, value, ficha, frags, question)
...
def topic_overview(subject: str, cfg=None, con=None, pool: int = 30,
                   per_party_k: int = 3, max_parties: int = 6) -> tuple[list[dict], str]:
    ...
    hits = retrieve(subject, cfg, None, final_k=pool, pool=pool)
...
def ask(query: str, cfg=None, con=None, final_k: int | None = None) -> tuple[dict, Recap, str]:
    ...
    if parsed.get("pleno"):
        ...
        recap = build_recap("candidato", parsed["candidato"], cfg, con, subject, final_k)
    elif parsed.get("partido"):
        recap = build_recap("partido", parsed["partido"], cfg, con, subject, final_k)
    else:                                            # sin entidad: búsqueda global
        frags = retrieve(subject, cfg, None, final_k) if subject else []
...
def _print_fragments_and_answer(recap: Recap, cfg, precomputed: str | None = None) -> None:
    ...
    for i, h in enumerate(recap.fragments, 1):
        m = h["meta"]
        print(f"\n[{i}] {m.get('speaker', '?')} · {m.get('party', '')} · {m.get('pleno_id', '')}")
...
def _emit_tema(subject, cfg, con, per_grupo=3, max_grupos=6) -> None:
    groups, resumen = topic_overview(subject, cfg, con,
                                     per_party_k=per_grupo, max_parties=max_grupos)
    if not groups:
...
def _emit_structured(kind, value, cfg, con, query=None, k=None, temas=False, max_temas=6) -> None:
    recap = build_recap(kind, value, cfg, con, question=query, final_k=k)
    print(format_ficha(recap))
    if recap.note:
        print(recap.note)
...
def main() -> None:
    ...
    if args.cmd == "repl":
        ...
        _emit_tema(args.subject, cfg, con, args.per_grupo, args.max_grupos)
    elif args.cmd == "ask":
        ...
        _emit_structured(args.cmd, args.value, cfg, con, query=args.query,
                         k=args.k, temas=args.temas, max_temas=args.max_temas)
    con.close()
